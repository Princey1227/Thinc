        ...
        <div
          className={`transition-all duration-300 ${
            isNavActive
              ...
              : 'bg-gradient-to-b from-black/60 via-black/20 to-transparent border-b border-white/10 py-4.5'
          }`}
        >
          <div className="max-w-7xl mx-auto px-6 md:px-12 flex items-center justify-between relative">
            
            {/* LEFT CORNER: SHOP, COLLECTIONS & ABOUT */}
            <div className="flex items-center gap-7 z-10">
              {/* SHOP button */}
              <button
                onClick={handleShopClick}
                className={`text-xs uppercase tracking-[0.25em] font-bold transition-colors py-1 cursor-pointer ${
                  isNavActive
              ...
              <div className="relative">
                <button
                  ...
                  className={`text-xs uppercase tracking-[0.25em] font-bold transition-colors py-1 cursor-pointer flex items-center gap-1.5 ${
                    isNavActive
                      ? 'text-[#0F172A] hover:text-red-600'
                      : 'text-white hover:text-amber-300 drop-shadow-md'
                  }`}
                ...
                <AnimatePresence>
                  {collectionsDropdownOpen && (
                    <motion.div
                      ...
                      className="absolute top-full left-0 mt-2 w-[540px] bg-white border border-gray-200 rounded-2xl shadow-2xl p-6 z-50 grid grid-cols-3 gap-6"
                    >
                      {/* MEN COLUMN */}
                      <div>
                        ...
                        <div className="flex flex-col gap-2">
                          {subCategories.map((sub) => (
                            <button
                              ...
                              className="text-xs text-left text-slate-600 hover:text-red-600 font-medium py-1 transition-colors hover:translate-x-1 duration-200 flex items-center justify-between"
                            >
                              <span>{sub}</span>
            ...
            <div className="absolute left-1/2 -translate-x-1/2 z-10">
              <a
                href="#"
                className={`flex items-center gap-2 text-2xl md:text-3xl font-serif font-extrabold tracking-[0.3em] transition-colors ${
                  isNavActive
                    ? 'text-[#0F172A] hover:text-red-600'
            ...
            {/* RIGHT CORNER: SEARCH, WISHLIST, BAG */}
            <div className="flex items-center gap-4 z-10">
              ...
              <button
                onClick={onOpenCart}
                className={`relative flex items-center gap-2 px-4 py-2 rounded-full transition-all text-xs tracking-wider uppercase font-semibold shadow-sm ${
                  isNavActive
              ...
              >
                ...
                <span className="w-4 h-4 bg-red-600 text-white rounded-full flex items-center justify-center text-[10px] font-bold">
                  {cartCount}
                </span>
              </button>
              ...
              <div className="flex items-center justify-between border-b border-gray-200 pb-4">
                <div className="flex items-center gap-3 flex-1">
                  ...
                  <input
                    ...
                    placeholder="Search THINC raw selvage collection..."
                    autoFocus
